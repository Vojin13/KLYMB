...
@section('content')
    <div class="max-w-2xl mx-auto space-y-8">
        <div>
            <h1 class="text-3xl font-black uppercase tracking-tighter text-gray-900">Edit Category</h1>
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $category->name ?? '') }}" class="w-full border-gray-300 p-2 text-sm" required>
                        </div>
                        <div>
                            <label class="block text-xs font-bold uppercase text-gray-500 mb-1">Slug</label>
                            <input type="text" name="slug" value="{{ old('slug', $category->slug ?? '') }}" class="w-full border-gray-300 p-2 text-sm" required>
                        </div>
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase text-gray-500 mb-1">Description</label>
                        <textarea name="description" rows="5" class="w-full border-gray-300 p-2 text-sm">{{ old('description', $category->description ?? '') }}</textarea>
                    </div>
