...
@section('content')
    <div class="space-y-8">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-black uppercase tracking-tighter text-gray-900">Categories</h1>
                <p class="text-gray-500 font-bold uppercase tracking-widest text-xs mt-1">Manage product organization</p>
            </div>
            <a href="{{ route('admin.categories.create') }}" class="bg-black text-white px-6 py-3 font-black uppercase tracking-widest hover:bg-red-600 transition text-xs">
                + Add New Category
            </a>
        </div>
        ...
        <div class="flex justify-start mb-4">
            <p class="text-gray-500 font-bold text-sm uppercase tracking-widest">
                Showing
                <span class="text-black">{{ $categories->firstItem() ?? 0 }}</span>
                to
                <span class="text-black">{{ $categories->lastItem() ?? 0 }}</span>
                of
                <span class="text-black">{{ $categories->total() }}</span>
                categories
            </p>
        </div>
        ...
        <div class="bg-white border border-gray-200 shadow-sm overflow-hidden rounded-sm">
            <table class="w-full text-left text-base">
                    ...
                    @foreach($categories as $c)
                        <tr class="{{ $loop->iteration % 2 == 0 ? 'bg-white' : 'bg-gray-50'}} hover:bg-gray-100 transition-colors">
                            ...
                            <td class="p-6 text-gray-600 text-sm">
                                {{ Str::limit($c->description, 40) ?? 'No description' }}
                            </td>
                            <td class="p-6 text-gray-600 text-sm">{{ $c->created_at->format('d. M. Y.') }}</td>
                            <td class="p-6 text-right">
                                <button id="dropdownDefaultButton-{{ $c->id }}"
                ...
                @else
                    <tr>
                        <td colspan="6" class="p-12 text-center text-gray-400 font-black uppercase tracking-widest">No categories found.</td>
                    </tr>
                @endif
                </tbody>
            </table>
