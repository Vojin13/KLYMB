<nav class="bg-neutral-primary h-[9vh] sticky w-full z-50 top-0 start-0 border-b border-gray-200 p-2">
    <div class="max-w-screen-xl flex items-center justify-between mx-auto px-4 h-full">
        ...
        <div class="flex items-center justify-end gap-x-4 w-[150px] flex-shrink-0">
            @auth
                <a href="{{ route('cart.index') }}" class="text-gray-700 hover:text-red-600 transition relative cursor-pointer">
                    ...
                    <span id="cart-count" class="absolute -top-1 -right-2 flex h-4 w-4 items-center justify-center rounded-full bg-red-600 text-[10px] font-bold text-white shadow-sm">{{ auth()->user()->cartItems->sum('quantity') }}</span>
                </a>
            @endauth
